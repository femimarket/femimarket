...
pub async fn transfers(configuration: &configuration::Configuration, id: &str) -> Result<models::Transfers200Response, Error<TransfersError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_query_id = id;

    let uri_str = format!("{}/api/debug/transfers", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::Transfers200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::Transfers200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<TransfersError> = serde_json::from_str(&content).ok();
