...
/// PlanOffset : A client-computed first/last mile option: the given transit stop can be reached from the query coordinate (or vice versa) with the given duration and mode. 
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct PlanOffset {
    /// stop id of the transit stop (parent stations are expanded to their child stops) 
    #[serde(rename = "stopId")]
    pub stop_id: String,
    /// duration in seconds
    #[serde(rename = "duration")]
    pub duration: i32,
    /// mode used to reach the stop, used to render the first/last mile leg (supported: \\`WALK\\`, \\`BIKE\\`, \\`CAR\\`) 
    #[serde(rename = "mode", skip_serializing_if = "Option::is_none")]
    pub mode: Option<models::Mode>,
}

impl PlanOffset {
    /// A client-computed first/last mile option: the given transit stop can be reached from the query coordinate (or vice versa) with the given duration and mode. 
    pub fn new(stop_id: String, duration: i32) -> PlanOffset {
        PlanOffset {
            stop_id,
            duration,
            mode: None,
