...
use localisation_service::commands;

#[derive(Parser)]
#[command(name = "localisation-service", about = "Localisation service")]
struct Cli {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Command::Init => {
            ...
            db.get_schema_registry("localisation_service::*").sync(&db).await?;
            println!("schema ready");
        }
        Command::Openapi => {
            use utoipa::OpenApi;
            println!("{}", localisation_service::server::ApiDoc::openapi().to_pretty_json()?);
        }
        Command::Serve { port } => {
