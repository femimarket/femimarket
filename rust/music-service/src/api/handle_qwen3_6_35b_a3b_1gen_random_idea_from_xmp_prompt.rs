...
pub async fn handle_qwen3_6_35b_a3b_1gen_random_idea_from_xmp_prompt(mut row: Model) -> Result<Model, (axum::http::StatusCode, String)> {
    let Model::Qwen3_6_35bA3b1GenRandomIdeaFromXmpPrompt { xmp_prompt, .. } = row.clone()
    else {
        return Err((axum::http::StatusCode::BAD_REQUEST, "handle_qwen3_6_35b_a3b_1gen_random_idea_from_xmp_prompt requires a Qwen3_6_35bA3b1GenRandomIdeaFromXmpPrompt action".to_string()));
    };

    let content = format!("Given this image-generation prompt, invent a fresh, random new idea as a single image-generation prompt. Reply with only the new prompt.\n\nPrompt:\n{xmp_prompt}");
    ...
    if let Model::Qwen3_6_35bA3b1GenRandomIdeaFromXmpPrompt { result, .. } = &mut row {
        *result = reply;
    }
    Ok(row)
}
