...
use ui_service::commands;

#[derive(Parser)]
#[command(name = "ui-service", about = "Ui service")]
struct Cli {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        Command::Init => {
            ...
            db.get_schema_registry("ui_service::*").sync(&db).await?;
            println!("schema ready");
        }
        Command::Openapi => {
            use utoipa::OpenApi;
            println!("{}", ui_service::server::ApiDoc::openapi().to_pretty_json()?);
        }
        Command::Serve { port } => {
