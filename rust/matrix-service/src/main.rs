use matrix_service::commands;
use matrix_service::server::{Cli, Command};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command.clone() {
        ...
        Command::Openapi => {
            use utoipa::OpenApi;
            println!("{}", matrix_service::server::ApiDoc::openapi().to_pretty_json()?);
        }
        Command::Serve { port } => {
            commands::serve::serve(cli, port).await?;
        }
    }
    Ok(())
}
